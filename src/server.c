...
void *tcp_accepter(void *arg)
{
	...
	while(1){
		//The check for the limit is done in cli_reciever (which allows the client to know why isn't being accepted), this is redundant
...
int main(int argc, char* argv[])
{
	...
	pthread_create(&id[2], NULL, computation, &messager);  	//starts thread that will compute 
	pthread_create(&id[1], NULL, bot_gen, &messager);  		//starts bot generating thread
